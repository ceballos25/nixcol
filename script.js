...
if (celularInput) {
  ...
  if (card) {
    ...
    btnComprarCard.addEventListener('click', (e) => {
      e.stopPropagation(); // Evita doble apertura si hay bubbling
      const cantidad = parseInt(cantidadInput.value, 10);
      ...
      // Pasa los valores al modal y lo muestra
  ...
  buttonComprar.forEach(button => {
    button.addEventListener('click', function () {
      // Solo ejecuta si el botón NO está dentro de .card-personalizado
  ...
  function setPackage(value) {
    ...
    if (value === 'otro') {
      totalInput.removeAttribute('readonly');
      // Si no hay valor previo, limpia y pone $0
      if (!totalInput.value || isNaN(totalInput.value)) {
        totalInput.value = '';
        totalPayInput.value = '$0';
      } else {
        // Si ya hay valor, actualiza el precio
  ...
  if (form) {
    form.addEventListener('submit', event => {
      const requiredFields = [
        'celular', 'nombre', 'email', 'usp-custom-departamento-de-residencia',
      ...
      if (!isValid) {
        ...
        // Si tienes un preloader, aquí lo puedes mostrar
